...
class GenerationMixin:
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        codefast_config_dict: Optional[Dict] = None, 
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        if codefast_config_dict != None:
            if codefast_config_dict['additional_model_type'] == 'linear':
                # genguard model
                genguard_model = codefast_config_dict['additional_model']

                # The token represents the end of a code line
                code_line_end_token = codefast_config_dict['code_line_end_token']
        
                is_convergence = codefast_config_dict['is_convergence']
                convergence_num = codefast_config_dict['convergence_num']
                continue_label =  codefast_config_dict['continue_label']
                stop_threshold = codefast_config_dict['stop_threshold']
                output_hidden_states=True
                pre_code_line_start_idx = None
                cur_code_line_start_idx = None
                empty_code_line_num = 1
                continual_stop_labels_num  = 0
        ...
        while True:
            ...
            if codefast_config_dict!=None:
                ...
                if is_convergence:
                    ...
                    max_value, max_index = torch.max(predict_logits, 1)
                    if max_index != continue_label and next_tokens[0] != code_line_end_token:
                        continual_stop_labels_num += 1
                    else:
                        continual_stop_labels_num = 0
                    # Stop inference if contunual stop labels are generated
                    if continual_stop_labels_num>=convergence_num:
                        unfinished_sequences[0] = 0
                        this_peer_finished = True
                    

                    if next_tokens[0] == code_line_end_token:
                        #update start index of previous code line 
                        pre_code_line_start_idx = cur_code_line_start_idx
                        #update start index of current code line 
                        cur_code_line_start_idx = input_ids.shape[1]
                        if pre_code_line_start_idx != None:
                            # find empty code line, empty_code_line_num += 1
                            if cur_code_line_start_idx == pre_code_line_start_idx +1:
                                empty_code_line_num += 1
                            # dont't find empty code line, reset empty_code_line_num 
                            else:
                                empty_code_line_num = 1

                        #Stop inference if more than 3 empty code lines are generated   
                ...
                #line voting mode
                    ...
                    if input_ids[0][-1] != code_line_end_token:
                        # compute last hidden states
                        next_tokens_hidden_states =  outputs.hidden_states[-1][:, -1, :]
                        ...
                        genguad_predict_labels_list.append(max_index.item())
                        
                    if next_tokens[0] == code_line_end_token:
                        # Determine whether to stop inference based on the proportion of 'stop' labels
                        if len(genguad_predict_labels_list)!=0:
                            if 2*genguad_predict_labels_list.count(continue_label)<len(genguad_predict_labels_list):
                                unfinished_sequences[0] = 0
                                this_peer_finished = True

                        #reset predict_labels_list before starting a new code line
                        genguad_predict_labels_list = []
            ...
            if this_peer_finished and not synced_gpus:
                # if codefast_config_dict!=None and is_convergence==False:
                #     if pre_code_line_start_idx!=None:
                #         input_ids = input_ids[:, :pre_code_line_start_idx]
                break
    ...
    def is_stopping(self,start_idx,output_ids,stop_word_ids):
        # print(f'original_length \n{len(output_ids)}')
        output_ids_list = output_ids.tolist()[start_idx:]
        # print(f'removed_length \n{len(output_ids_list)}')
        is_stopping = False
        max_idx = len(output_ids_list)-1
        # print(max_idx)
        # input()
        for output_id in range(len(output_ids_list)):
            if output_ids_list[output_id] == 13:
                for stop_word_id in stop_word_ids:
                    
                    if len(stop_word_id)+output_id<=len(output_ids_list)-1:
                        is_find =True
                        temp_idx = 1
                        
                        for stop_word in stop_word_id:
                            if stop_word != output_ids_list[temp_idx+output_id]:
                                is_find =False
                                break
                            else:
                                temp_idx += 1
                        if is_find:
                            is_stopping =True 
                            max_idx = min(max_idx,output_id)
                            if max_idx<len(output_ids_list)-1:
                                break
                            

        return is_stopping,max_idx
